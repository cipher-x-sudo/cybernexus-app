...
class TorConnector:
    ...
    def check_yara(self, raw: str, yarafile: str) -> List:
        ...
        try:
            ...
            self.logger.warning(f"YARA rule check failed: {e}")
            return []
    ...
    def crawler(self, url: List) -> Dict[str, Any]:
        ...
        try:
            ...
            if request.status_code == 200:
                self.logger.info(f"Updating url status {url_str} for 200.")
                ...
                self.logger.info(f"Updating url status {url_str} for {request.status_code}.")
                self.database.update_status(
                    id=url_id,
                    url=url_str,
                    result=404,
            ...
            self.logger.info(f"Could not connect to site. Updating url status {url_str} for 404.")
            self.database.update_status(
                id=url_id,
                url=url_str,
                result=404,
